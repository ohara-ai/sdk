...
import { LEAGUE_FACTORY_ABI } from '../abis/game/leagueFactory'
import {
...
/**
 * Deploy a League contract instance
 * Supports both direct on-chain and Ohara API modes
 */
export async function deployLeague(
  params: LeagueDeployParams,
): Promise<DeploymentResult> {
  const config = getConfig()

  const factoryAddress = config.factories.league
  if (!factoryAddress) {
    throw new Error('League factory address not configured')
  }
  ...
  const chainId = await publicClient.getChainId()
  try {
    await setContractAddress('game', 'league', deployedAddress, chainId)
  } catch (storageError) {
    console.error('Failed to save address to backend storage:', storageError)
  }
