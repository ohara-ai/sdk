...
export async function POST(request: NextRequest) {
  try {
    ...
    // Call cancel operation from SDK (controller-only)
    const success = await game.heap.operations.cancel(BigInt(heapId))
    ...
    console.error('Heap cancellation error:', error)
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : 'Failed to cancel heap',
      },
      { status: 500 },
    )
  }
