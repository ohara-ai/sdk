...
const CONTRACT_MAPPINGS = [
  ...
  {
    name: 'Tournament',
    output: 'game/tournament',
    type: 'local',
    exportName: 'TOURNAMENT_ABI',
  },
  {
    name: 'TournamentFactory',
    output: 'game/tournamentFactory',
    type: 'local',
...
function updateIndexFile() {
  ...
  const gameExports = CONTRACT_MAPPINGS.filter((contract) =>
    contract.output.startsWith('game/'),
  )
  ...
  const exports = ['export * from "./game";', 'export * from "./erc";'].join(
    '\n',
  )
...
function main() {
  ...
  if (!fs.existsSync(CONTRACTS_OUT_DIR)) {
    console.error(
      '❌ Contracts not compiled. Run "cd contracts && forge build" first.',
    )
    process.exit(1)
  }
