...
contract ScoreBoardTest is Test {
    ...
    function setUp() public {
        ...
        scoreBoard = new ScoreBoard(owner, 50, 1000, 100);
    }
    ...
    function test_UpdateLimits() public {
        ...
        scoreBoard.updateLimits(100, 20000, 200000);
        
        assertEq(scoreBoard.maxLosersPerMatch(), 100);
        assertEq(scoreBoard.maxTotalPlayers(), 20000);
        assertEq(scoreBoard.maxTotalMatches(), 200000);
    }
    ...
    function test_CannotReducePlayerLimitBelowCurrent() public {
        ...
        vm.prank(owner);
        vm.expectRevert(ScoreBoard.InvalidLimit.selector);
        scoreBoard.updateLimits(0, 1, 0); // Try to set maxTotalPlayers to 1
    }
    ...
    function test_CannotReduceMatchLimitBelowCurrent() public {
        ...
        vm.prank(owner);
        vm.expectRevert(ScoreBoard.InvalidLimit.selector);
        scoreBoard.updateLimits(0, 0, 1); // Try to set maxTotalMatches to 1
    }
    ...
    function test_LosersWithinLimitNotTruncated() public {
        ...
        (, address[] memory recordedLosers, , ) = scoreBoard.getMatchRecord(1);
        assertEq(recordedLosers.length, 50);
        
        // Total players should be 51 (winner + 50 losers)
        assertEq(scoreBoard.getTotalPlayers(), 51);
    }
    ...
    function test_GetRemainingCapacity() public {
        assertEq(scoreBoard.getRemainingPlayerCapacity(), 1000);
        assertEq(scoreBoard.getRemainingMatchCapacity(), 100);
        ...
        assertEq(scoreBoard.getRemainingPlayerCapacity(), 998); // 2 players added
        assertEq(scoreBoard.getRemainingMatchCapacity(), 99); // 1 match added
    }

    function test_MatchEvictionWhenAtCapacity() public {
        // Create scoreboard with small match limit
        vm.prank(owner);
        ScoreBoard smallScoreBoard = new ScoreBoard(owner, 50, 1000, 3); // Only 3 matches
        
        vm.prank(owner);
        smallScoreBoard.setRecorderAuthorization(recorder, true);
        ...
        vm.prank(recorder);
        smallScoreBoard.recordMatchResult(1, player1, losers, 100 ether);
        vm.prank(recorder);
        smallScoreBoard.recordMatchResult(2, player1, losers, 100 ether);
        vm.prank(recorder);
        smallScoreBoard.recordMatchResult(3, player1, losers, 100 ether);
        
        assertEq(smallScoreBoard.getTotalMatches(), 3);
        
        // Record 4th match - should evict oldest (match 1)
        vm.prank(recorder);
        vm.expectEmit(true, false, false, false);
        emit MatchEvicted(1, block.timestamp);
        smallScoreBoard.recordMatchResult(4, player1, losers, 100 ether);
        
        assertEq(smallScoreBoard.getTotalMatches(), 3); // Still at capacity
        
        // Match 1 should be gone, but match 2, 3, 4 should exist
        (, , , uint256 timestamp1) = smallScoreBoard.getMatchRecord(1);
        assertEq(timestamp1, 0); // Match 1 evicted
        
        (, , , uint256 timestamp2) = smallScoreBoard.getMatchRecord(2);
    ...
    function test_PlayerEvictionWhenAtCapacity() public {
        ...
        vm.prank(owner);
        ScoreBoard smallScoreBoard = new ScoreBoard(owner, 50, 3, 1000); // Only 3 players
        ...
        losers[0] = player3;
        vm.prank(recorder);
        smallScoreBoard.recordMatchResult(1, player1, losers, 200 ether);
        ...
        losers[0] = player3;
        vm.prank(recorder);
        smallScoreBoard.recordMatchResult(3, player2, losers, 150 ether);
        
        assertEq(smallScoreBoard.getTotalPlayers(), 3);
        ...
        smallScoreBoard.recordMatchResult(4, player4, losers, 50 ether);
        
        assertEq(smallScoreBoard.getTotalPlayers(), 3); // Still at capacity
        
        (uint256 p3Wins, , , ) = smallScoreBoard.getPlayerScore(player3);
        assertEq(p3Wins, 0); // Player3 evicted
        
        (uint256 p4Wins, , , ) = smallScoreBoard.getPlayerScore(player4);
    ...
    function test_PlayerEvictionTieBreaker() public {
        // Create scoreboard with small player limit
        vm.prank(owner);
        ScoreBoard smallScoreBoard = new ScoreBoard(owner, 50, 3, 1000);
        ...
        vm.prank(recorder);
        smallScoreBoard.recordMatchResult(1, player1, losers, 300 ether); // 1 win, 300 ether
        
        losers[0] = address(0x999);
        vm.prank(recorder);
        smallScoreBoard.recordMatchResult(2, player2, losers, 200 ether); // 1 win, 200 ether
        
        losers[0] = address(0x999);
        vm.prank(recorder);
        smallScoreBoard.recordMatchResult(3, player3, losers, 100 ether); // 1 win, 100 ether
